...
| S12 | セッション本体 1,166 件（全数。うち repo 未導出 538 件）             | O30                             |
| S13 | セッション本体 1,225 件（全数）のうち assistant レコード 5 件以上のセッション 677 件 | O31                             |
...
| O30 | S12 | repo 未導出の 538 件のうち、開始時 cwd のディレクトリは **95.4%（513 件）が今も存在する**。しかしそのうち **git の作業ツリーであるものは 9 件（1.7%）のみ**（`git rev-parse --show-toplevel` で判定）。残る 504 件は cwd が git リポジトリの外（ホーム直下・一時ディレクトリ等）を指しており、`git remote` を引ける状態ではない。9 件全件で origin remote から owner/repo を復元できたが、全体の導出可能率への効果は 53.7% → 54.5%（+0.8pt）に留まる | **ディレクトリの残存率は高いが、未導出の主因はディレクトリの消失ではなく cwd がそもそも git リポジトリの外を指していること**。`git remote` 補完は投資対効果が低く、9.3 の課題として不採用と判断できる |
| O31 | S13 | 全 assistant レコードで合計すると、`input_tokens` はプロンプト系トークン合計（`input` + `cache_read` + `cache_creation`）の **0.0%**（4.3M）を占めるに過ぎず、**`cache_read_input_tokens` が 97.6%**（39.0B）、`cache_creation_input_tokens` が 2.4%（963M）を占める。5 ターン以上のセッション 677 件のうち、ターンを追うごとに `input_tokens` が単調非減少（累積文脈仮説）なのは **39.1%** のみで、60.9% はそうではない。最終ターンでは `cache_read_input_tokens` がそのターンのプロンプトトークンの中央値 **99.7%** を占め、セッション内 `input_tokens` の合計は最終ターンの文脈量に対して中央値 **0.19%** しかない | **`input_tokens` はターンごとの文脈全体を含まない**。プロンプトキャッシュにより過去の文脈はほぼ全量 `cache_read_input_tokens` に載り、`input_tokens` はキャッシュに乗らなかった新規分（ほぼゼロに近い）を表す。9.3 が想定していた「`input_tokens` の単純合計 = 投入した文脈量の累計」という前提は実データでは成り立たない |
...
| repo の導出可能率が 53%（O23） | `git remote` による補完を実データで検証し、効果が乏しいことを確認した（+0.8pt。O30 / D22）。53% を上限として受け入れ、追加実装は行わない |
| トークン使用量の解釈 | `input_tokens` はプロンプトキャッシュにより全体の 0.0% しか占めず、文脈量の指標にならないことを実データで確認した。`cache_read_tokens`（再利用）/ `cache_creation_tokens`（新規導入）で解釈する方針を確定した（O31 / D23） |
...
9.3 に繰り越していた「トークン使用量の解釈」の課題を調査し、解決済みへ移動した。

**実データでの観測（O31 / 標本 S13 = セッション本体 1,225 件のうち assistant レコード 5 件以上の 677 件）:**

全 assistant レコードでプロンプト系トークンを合計すると、`input_tokens` は全体の **0.0%** しか
占めず、`cache_read_input_tokens` が **97.6%**、`cache_creation_input_tokens` が **2.4%** を占めた。
セッションを追跡すると、`input_tokens` がターンを追うごとに単調非減少（累積文脈仮説）なのは
39.1% のみで、最終ターンの `cache_read_input_tokens` はそのターンのプロンプトトークンの
中央値 99.7% を占めた。

**判断（D23）**: `input_tokens` の単純合計を「投入した文脈量」の指標として使わない。
文脈の推移は `cache_read_tokens`（再利用分）と `cache_creation_tokens`（新規導入分）に現れるため、
これらを使って解釈する。コストを再構成する場合は 4 種すべてを実際の課金レートで重み付けする。

**教訓**: 9.3 の当初の想定（「`input_tokens` はターンごとに文脈全体を含む」）はプロンプト
キャッシュの効果を考慮していなかった。実データを見るまでは「もっともらしい仮説」と
「実際の挙動」の区別がつかない、という本プロジェクトで繰り返し確認してきた教訓が再確認された。

## v0.8（2026-08-17）

9.3 に繰り越していた「repo の導出可能率 53%」の課題を調査し、解決済みへ移動した。

**実データでの観測（O30 / 標本 S12 = 全セッション本体 1,166 件のうち repo 未導出 538 件）:**

repo 未導出セッションの開始時 cwd ディレクトリは 95.4%（513 件）が今も存在するが、
そのうち git の作業ツリーであるものは 1.7%（9 件）のみで、残り 504 件は cwd が
そもそも git リポジトリの外を指していた。9 件全件で `git remote` から owner/repo を
復元できたが、全体の導出可能率への効果は 53.7% → 54.5%（+0.8pt）に留まる。

**判断（D22）**: `git remote` による補完は実装コストに対して効果が乏しく、不採用とする。
53% を導出可能率の上限として受け入れる。

**教訓**: 「ディレクトリが消えているはず」という当初の想定（9.3 の懸念）は実データでは
成り立たず、真因は別（cwd が git リポジトリの外）だった。対応方針を決める前に
実データで仮説を検証する、という本プロジェクトの一貫した進め方が今回も差分を防いだ。
