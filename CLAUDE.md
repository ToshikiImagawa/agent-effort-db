...
**CLI 一式は実装済み**（`init` / `backfill sessions` / `backfill prs` / `collect-session` / `link` / `stats` / `query`）。
突き合わせは `(repo, branch)` 主軸 + ログ内 PR 参照優先で動き、由来を `link_source` に残す。
**実測データに基づく設計判断への追随は 9 項目すべて完了**し、**M0〜M8 も完了**した（`_design.md` の 1.1 参照）。
実データでの join 率は PR 収集済みリポジトリで 99.5%（従来のチケットキー単独では 0%）。
繰り越し中の課題は **repo の導出可能率が 53%**（`_design.md` 9.3）とトークンの解釈。[ROADMAP.md](ROADMAP.md) を参照すること。
...
- セッション ↔ PR の join キーは **`(repo, branch)` を主軸**とし、セッションログ内の PR 参照が得られる場合はそれを優先する。チケットキーは補助的な集約キーであり、単独の主キーとしない（形式は `config.toml` から与える。ドキュメント・コード中に実在のプロジェクトキーを書かない）
    - 実ログ調査の結果、ブランチ名にチケットキーを含むものは 1 件も無く、チケットキー主軸では集約が成立しないことが判明した。実測でも join 率は 0% だった。観測結果は `_design.md` の 9.1、判断の根拠は 9.2（D6）にある
...
│   ├── linker.py           # 段階適用 / 由来の記録 / issue_key 抽出（実装済み）
│   └── stats.py            # 由来ごとの join 率など健全性集計（実装済み）
└── tests/
    ├── fixtures/projects/  # 合成jsonlのみ（実ログのコピー禁止）
    └── test_*.py           # 94 件
```

`stats.py`（健全性集計）は実装済み。
**`models.py` は作らない**。型はそれを生み出すモジュールに置く（`SessionRecord` は `collectors/session.py`、
`LinkSource` は `linker.py`）。型専用モジュールを設けると依存方向が二重になるだけで、得るものが名前空間の分離しかない。
...
`query` は `PRAGMA query_only` で読み取り専用にする（逃げ道の SQL で raw 層を壊せないようにする）。
...
effort-db backfill sessions         # 過去jsonl走査（実装済み。常に全走査：8.4秒で完走するため増分判定は持たない）
effort-db backfill prs --repo xxx   # gh経由でPR取得（実装済み）
effort-db collect-session <id>      # インクリメンタル用（実装済み。将来hookから）
effort-db link                      # 突き合わせを段階適用（収集とは独立）
effort-db stats                     # 由来ごとのjoin率など健全性確認
effort-db query "..."               # 素のSQL逃げ道（読み取り専用）
```

## 実装時の注意点

- **実ログ構造の調査は完了している**。観測結果は `_design.md` 9.1（O1〜O25）、そこから導いた判断は 9.2（D1〜D21）にある。実装前に必ず参照すること。推測で作り直さない
...
- 特に注意すべき観測事実:
    ...
    - ツール呼び出しの 26.1% がサブエージェント内（出力トークンは 12.4%）。主エージェントと別列で保持する（D2 / O26）
    ...
    - ログ内 PR 参照（`type: pr-link`）はセッションの 43% が持つ。`prRepository` は `owner/repo` 形式で得られる（O18）
    - トークンは 4 種（入力 / 出力 / キャッシュ読み取り / キャッシュ作成）すべてが全 assistant レコードに存在する（O20）。サブエージェント分は output の約 12% を占めるため別列で持つ（O21 / D19）
